...
client.on('message', (message) => {
  ...
  if(message.content == '#($#ㅉ()_$)_#$(ㅉ)_()$)#$') {
    ...
    message.channel.send(embed)
  } else if(message.content == '!)_(!)(_!()_!)(_드2') {
  ...
  } else if(message.content.startsWith('@#@*#*@ㅃ)*(#')) {
    if(checkPermission(message)) return
    if(message.member != null) { // 채널에서 공지 쓸 때
    ...
    var clearLine = message.content.slice('!@()$()@()@$)소 '.length);
...
function checkPermission(message) {
  if(!message.member.hasPermission("MANAGE_MESSAGES")) {
    message.channel.send(`<@${message.author.id}> ` + "명령어를 실행하긴 위해선 관리자 권한이 필요합니다.")
    return true;
  } else {
    return false;
  }
